...
* 移動する操作対象の仮想物体の選択
*/
int DetermineTransObject(double hand_center_x, double hand_center_y, double hand_center_z,
	float pos_x_tmp, float pos_y_tmp, float pos_z_tmp,
	float pos_x, float pos_y, float pos_z) {

	float length1 = pow(hand_center_x - pos_x_tmp, 2) +
		pow(hand_center_y - pos_y_tmp, 2) +
		pow(hand_center_z - pos_z_tmp, 2);

	float length2 = pow(hand_center_x - pos_x, 2) +
		pow(hand_center_y - pos_y, 2) +
		pow(hand_center_z - pos_z, 2);
	...
	if (g_pDynamicsWorld) {
		...
		for (int i = 0; i < n; ++i) {
			...
			if (shapetype == SOFTBODY_SHAPE_PROXYTYPE) {
				...
				if (trans_obj == -1 && n > 3 && i == 5) {
					trans_obj = DetermineTransObject(metacarpal_x[2], metacarpal_y[2], metacarpal_z[2],
						pos_x_tmp, pos_y_tmp, pos_z_tmp,
						pos[0], pos[1], pos[2]);

				}
				else if (trans_obj == -1 && n <= 3) {
					trans_obj = 1;
				}
				...
				if ((i == 3 && trans_obj == 0) &&
					...
					sphere[trans_obj]->setMass(5, 0.0f);  // 固定
					trans_obj = -1;
